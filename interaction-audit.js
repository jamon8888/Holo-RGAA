#!/usr/bin/env node
/**
 * Phase 2: Interaction-based RGAA Testing
...
/**
 * 1. KEYBOARD SIMULATION - Tab/Shift+Tab sequential navigation
 * Tests: 10.7 (focus visible), 12.8 (tabindex), 12.9 (keyboard traps)
 */
async function runKeyboardSimulation(page, url) {
  console.log(`\n⌨️  Keyboard Simulation: ${url}`);
  ...
  while (iterations < maxIterations) {
    ...
    // Detect keyboard traps (12.9) - conservative detection
    // Only flag actual traps: focus stuck in modal/dialog without escape
    const elementKey = `${activeElement.tagName}#${activeElement.id}.${activeElement.className}`;
    tabOrder.push(elementKey);
    
    // Check if we're in a modal/dialog and focus is trapped
    // (This is better tested with the Escape key test)
    
    // Normal completion: we've tabbed through elements and hit body
    ...
    // Break if we're back at body (end of tab order)
    const isBody = await page.evaluate(() => document.activeElement === document.body);
    if (isBody) break;
  }
  ...
  // Test Escape key for modal/dialog dismissal (12.11)
  ...
  console.log(`   Tab order length: ${tabOrder.length}`);
  console.log(`   10.7 Focus visible: ${results['10.7'].passed ? 'PASS' : 'FAIL'} (${results['10.7'].missingFocus.length} issues)`);
  console.log(`   12.8 Tabindex: ${results['12.8'].passed ? 'PASS' : 'FAIL'} (${results['12.8'].tabindexIssues.length} issues)`);
  console.log(`   12.9 Keyboard traps: ${results['12.9'].passed ? 'PASS' : 'FAIL'} (${results['12.9'].keyboardTraps.length} issues)`);
  console.log(`   12.11 Escape dismissal: ${results['12.11'].passed ? 'PASS' : 'FAIL'} (${results['12.11'].keyboardOperable.length} issues)`);
  
  return results;
}

/**
 * 2. DOM vs VISUAL READING ORDER - Criterion 9.3
 * Compares DOM order vs visual (bounding box) order
 */
async function runReadingOrderTest(page, url) {
  console.log(`\n📖 Reading Order Test: ${url}`);
  ...
  const visualOrder = [...readingOrderData].sort((a, b) => {
    ...
    if (Math.abs(topDiff) > 10) return topDiff; // Different lines
    return a.rect.left - b.rect.left; // Same line, left-to-right
  });
  
  // Compare DOM order vs visual order
  const domOrder = [...readingOrderData].sort((a, b) => a.domIndex - b.domIndex);
  
  // Find significant discrepancies - only flag if element shifts by more than 5 positions
  // AND the element is a major structural element (heading, landmark, etc.)
  ...
  for (let i = 0; i < Math.min(domOrder.length, visualOrder.length); i++) {
    ...
    // Only check structural elements for reading order
  ...
  console.log(`   Elements analyzed: ${readingOrderData.length}`);
  console.log(`   9.3 Reading order: ${results['9.3'].passed ? 'PASS' : 'FAIL'} (${results['9.3'].discrepancies.length} discrepancies)`);
  
  return results;
}

/**
 * 3. REFLOW/ZOOM 200% TEST - Criterion 10.11
 * Tests content reflow at 200% zoom (equivalent to 320px width)
 */
async function runReflowTest(page, url) {
  console.log(`\n🔍 Reflow/Zoom 200% Test: ${url}`);
  ...
  for (const viewport of viewports) {
    ...
    const reflowIssues = await page.evaluate(() => {
      ...
      // Check for horizontal scroll (reflow failure)
      ...
      // Check text spacing (10.12) - at high zoom
    ...
    console.log(`   ${viewport.name} (${viewport.width}px): ${reflowIssues.length} issues`);
  }
  
  // Reset viewport
  await page.setViewportSize({ width: 1280, height: 720 });
  
  console.log(`   10.11 Reflow: ${results['10.11'].passed ? 'PASS' : 'FAIL'}`);
  console.log(`   10.12 Text spacing: ${results['10.12'].passed ? 'PASS' : 'FAIL'}`);
  
  return results;
}

/**
 * 4. FORM SUBMISSION TESTING - Theme 11
 * Tests: 11.1 (labels), 11.4 (instructions), 11.5/11.6 (fieldset), 
 *        11.11 (error suggestion), 11.12 (error prevention), 11.13 (autocomplete)
 */
async function runFormSubmissionTest(page, url) {
  console.log(`\n📝 Form Submission Test: ${url}`);
  ...
  const formData = await page.evaluate(() => {
    ...
    forms.forEach((form, formIndex) => {
      ...
      // Check labels using DOM elements
  ...
  for (const form of formData) {
    ...
    // 11.4 - Instructions (aria-describedby for required/error info)
    form.inputs.forEach(input => {
    ...
    // 11.5/11.6 - Fieldset/legend for grouped controls
    if (form.fieldsets.length === 0 && form.inputs.length > 3) {
      // Check if there are radio/checkbox groups that should be fieldset
    ...
    if (form.submitButtons.length > 0) {
      try {
        ...
        // Check for error messages and aria-invalid
        ...
        if (submissionResult.length === 0) {
          ...
          // Check if error messages are helpful (11.11)
        ...
        // 11.12 - Error prevention (check for confirmation on irreversible actions)
        ...
        console.log(`   Form submission test error: ${e.message}`);
      }
    }
  }
  
  console.log(`   Forms found: ${formData.length}`);
  console.log(`   11.1 Labels: ${results['11.1'].passed ? 'PASS' : 'FAIL'} (${results['11.1'].missingLabels.length} issues)`);
  console.log(`   11.4 Instructions: ${results['11.4'].passed ? 'PASS' : 'FAIL'} (${results['11.4'].missingInstructions.length} issues)`);
  console.log(`   11.5/11.6 Fieldset: ${results['11.5'].passed ? 'PASS' : 'FAIL'} (${results['11.5'].fieldsetIssues.length} issues)`);
  console.log(`   11.11 Error handling: ${results['11.11'].passed ? 'PASS' : 'FAIL'} (${results['11.11'].errorHandling.length} issues)`);
  console.log(`   11.12 Error prevention: ${results['11.12'].passed ? 'PASS' : 'FAIL'} (${results['11.12'].errorPrevention.length} issues)`);
  console.log(`   11.13 Autocomplete: ${results['11.13'].passed ? 'PASS' : 'FAIL'} (${results['11.13'].autocompleteIssues.length} issues)`);
  
  return results;
}

/**
 * Run all Phase 2 interaction tests
 */
async function runInteractionAudit(url) {
  console.log(`\n═══════════════════════════════════════════════════════════════`);
  console.log(`🔬 PHASE 2 INTERACTION AUDIT: ${url}`);
  console.log(`═══════════════════════════════════════════════════════════════`);
  ...
  const criteria = ['10.7', '12.8', '12.9', '12.11', '9.3', '10.11', '10.12', '11.1', '11.4', '11.5', '11.6', '11.11', '11.12', '11.13'];
  let passed = 0, failed = 0;
  
  for (const criterion of criteria) {
    if (allResults[criterion]) {
      const status = allResults[criterion].passed ? '✅ PASS' : '❌ FAIL';
      if (allResults[criterion].passed) passed++; else failed++;
      console.log(`   ${criterion}: ${status}`);
    }
  }
  
  console.log(`\n   Total: ${passed} passed, ${failed} failed out of ${criteria.length} criteria tested`);
