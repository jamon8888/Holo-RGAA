...
fn findings_from_axe(value: &serde_json::Value) -> Result<Vec<rgaa_core::Finding>, ObscuraError> {
    ...
    let json = serde_json::to_string(array).map_err(|e| ObscuraError::Json(e.to_string()))?;
    let mapping =
        rgaa_rules::AxeMapper::map(&json).map_err(|e| ObscuraError::Evaluation(e.to_string()))?;
    let findings = mapping
        .values()
        .filter(|c| matches!(c.status, rgaa_core::CriterionStatus::Fail))
        .map(|c| rgaa_core::Finding::new(c.criterion_id.clone()))
        .collect();
    Ok(findings)
}
...
impl BrowserWorker {
    fn new(
        ...
        let runtime = tokio::runtime::Runtime::new()
            .map_err(|e| ObscuraError::ProcessStartup(format!("failed to create tokio runtime: {e}")))?;
        Ok(Self {
            browser,
            axe_source,
            rx,
    ...
    fn run(mut self) {
        while let Ok((request, reply)) = self.rx.recv() {
            ...
            let response = match request {
                BrowserRequest::Navigate(url) => self.handle_navigate(&url),
    ...
    fn handle_navigate(&mut self, url: &str) -> BrowserResponse {
        let result = self.runtime.block_on(async {
            let mut page =
                self.browser.new_page().await.map_err(|e| {
...
impl BrowserHandle {
    ...
    pub fn type_input(&self, selector: &str, text: &str) -> Result<(), ObscuraError> {
        match self.send(BrowserRequest::TypeInput(
            selector.to_string(),
            text.to_string(),
        ))? {
            BrowserResponse::Unit(()) => Ok(()),
...
impl ObscuraNative {
    ...
    pub async fn analyze(
        ...
        self.handle
            .send(BrowserRequest::Analyze(request.clone()))
    ...
    async fn execute(&mut self, action: &GuidedAction) -> Result<GuidedObservation, ObscuraError> {
        ...
        match action {
            GuidedAction::Navigate { url } => {
                let resp = self
                    .handle
                    .send_async(BrowserRequest::Navigate(url.clone()))
                    .await?;
                match resp {
...
async fn test_native_creation() {
        // Creation requires browser and network access
